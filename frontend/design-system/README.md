...
**Synced project:** `Claude Karma Design System` — projectId `5fdd2fba-3802-4151-98ba-586d685cebf5`. Initial sync 2026-06-23 (24 files: 21 cards + `shared/{tokens,preview}.css` + `mapping.json`); Pages layer added same day (`pages/cron.html`, `pages/shells.html` → 23 cards total). The remote also auto-generates `_ds_manifest.json` / `_ds_bundle.js` / `_adherence.oxlintrc.json` (the app's self-check — leave them). For incremental re-syncs, run `npm run ds:build`, then `DesignSync` `list_files` → `finalize_plan` (writes: `foundations/*.html`, `components/*.html`, `pages/*.html`, `shared/*.css`, `mapping.json`; `localDir` = this `catalog/`) → `write_files` — upload only the files that changed.
